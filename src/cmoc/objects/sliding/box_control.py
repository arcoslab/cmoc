...
def calc_finger_start_pos(
        finger_offset, touch_point, object_pos, touch_face, box_planes):
    touch_point_local = dot(
        inv(object_pos), homo_matrix(trans=touch_point))[:3, 3]
    print 'touch local', touch_point_local, 'plane', box_planes[touch_face][0]
    return (
        dot(
            object_pos,
            homo_matrix(
                trans=touch_point_local +
                array(box_planes[touch_face][0]) * finger_offset))[:3, 3])
...
def calc_finger_orient(finger_base_orient, table_normal, object_pos, face):
    ...
    return (
        dot(
            object_pos[:3, :3],
            dot(rot_vector_angle(table_normal, angle), finger_base_orient)))


def approach_until_touch(
        hand_handle,
        finger,
        arm_handle,
        force_handle,
    ...
    filters_order = 3
    filters_freq = 0.2
    max_noise_force, max_noise_norm_force = find_force_noise_levels(
        force_handle,
        finger,
        filter_order=filters_order,
        filter_freq=filters_freq,
        measurements=50)
