...
def forces_to_no_map(fmax, mmax):
    A = array(
        [
            [2 / fmax**2, 0, 0],  # maps forces to nc
            [0, 2 / fmax**2, 0],
            [0, 0, 2 / mmax**2]
        ])
    return (A)
...
def nc_to_forces_map(fmax, mmax, contact_local):
    a = (
        4 * (mmax**2 / fmax**2 + contact_local[1]**2) *
        (mmax**2 / fmax**2 + contact_local[0]**2) -
    ...
    B_inv = (2. / a) * array(
        [
            [
                mmax**2 / fmax**2 + contact_local[0]**2,
                contact_local[0] * contact_local[1]
            ], [
                contact_local[0] * contact_local[1],
                mmax**2 / fmax**2 + contact_local[1]**2
            ]
        ])
    return (B_inv)
...
def force_c_to_force_o_map(contact_local):
    C = array(
        [
            [1, 0],  # maps elipsoid to contact point
            [0, 1],
...
def fc_to_nc_matrix(fmax, mmax, contact_pos):
    m = 2 * array(
        [
            [
                mmax**2 / fmax**2 + contact_pos[1]**2,
                -contact_pos[0] * contact_pos[1]
            ], [
                -contact_pos[0] * contact_pos[1],
                mmax**2 / fmax**2 + contact_pos[0]**2
            ]
        ])
    return (m)
...
def norm_nc_under_vc(fmax, mmax, contact_pos, fc2):
    nc = mmax * sqrt(
        1 / (
            (mmax**2 / fmax**2 + contact_pos[1]**2) * fc2[0]**2 +
...
def vo_to_vc_map(contact_local):
    D = array([[1, 0, -contact_local[1]], [0, 1, contact_local[0]]])
    return (D)


def ellipse_in_c(mmax, fmax, contact_local, fc_local):
    one = (1 / mmax**2) * (
        (mmax**2 / fmax**2 + contact_local[1]**2) * fc_local[0]**2 +
...
def nc_max_min(
        table_face_normal, finger_face_normal, friction_coef_finger_object,
        LC_c_matrix):
    # rotation is over the table_face_normal_axis
    # We rotate finger_face_normal by arctan(friction_coef) to obtain
    # corresponding forces.
    # We apply the this forces through B to obtain nc_max_min
    rot_mat = rot_vector_angle(
        -table_face_normal, arctan(friction_coef_finger_object))
...
def vc_from_vfinger(
        vfinger, finger_pos, box_vertices, box_planes, finger_face_normal,
    ...
    nc_max, nc_min, fc_max, fc_min = nc_max_min(
        table_face_normal, finger_face_normal, friction_coef_finger_object,
        inv(LC_c_matrix))  # TODO change to non inverse
    ...
    if w1 < 0:
        ...
        vc = (
            nc_min * dot(vfinger, -finger_face_normal) / dot(
                nc_min, -finger_face_normal))
        # projection of vfinger on nc_min that
    ...
    for face in vertices_faces:
        temp = array(
            polygon_pluecker_test(
                vertices_faces[face], pos_finger, vfinger / norm(vfinger)))
        ...
        if all(temp > 0.) or all(temp < 0.):
            ...
            while i < len(temp) - j:
                ...
                if num == 0. and not crossing_face:
                    ...
                    # we rectify wrong previews assumption
                    crossing_face = False
                    break
                else:
                    i += 1
        if crossing_face:
            ...
            intersection = plane_ray_intersec2(
                array(box_planes[face][0]), array(box_planes[face][1]),
                vfinger, pos_finger)
