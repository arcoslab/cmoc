...
from numpy import (
    arange, pi, sin, cos, tan, array, arctan2, arctan, dot, arcsinh)
...
fay = lambda u,p,cx,cy,x,y: u*p*(x-cx)/sqrt((x-cx)*(x-cx)+(y-cy)*(y-cy)) # noqa
fax = lambda u,p,cx,cy,x,y: u*p*(cy-y)/sqrt((x-cx)*(x-cx)+(y-cy)*(y-cy)) # noqa


def fx(l1l, l1h, l2l, l2h, cx, cy, u, p, eps=0.00001):
    fax2 = lambda y, x: fax(u, p, cx, cy, x, y) # noqa
    return (
        dblquad(
            fax2,
            l1l,
...
def fy(l1l, l1h, l2l, l2h, cx, cy, u, p, eps=0.00001):
    fay2 = lambda y, x: fay(u, p, cx, cy, x, y) # noqa
    return (
        dblquad(
            fay2,
            l1l,
...
def m(l1l, l1h, l2l, l2h, cx, cy, u, p, eps=0.00001):
    fay2 = lambda x, y: fay(u, p, cx, cy, x, y) # noqa
    fax2 = lambda x, y: fax(u, p, cx, cy, x, y) # noqa
    m = lambda y, x: x * fay2(x, y) - y * fax2(x, y) # noqa
    return (
        dblquad(
            m, l1l, l1h, lambda x: l2l, lambda x: l2h, epsabs=eps, epsrel=eps)) # noqa


def LS_point((l1l, l1h, l2l, l2h, cx, cy, u, p), eps=0.1):
    return (
        (
            fx(l1l, l1h, l2l, l2h, cx, cy, u, p, eps=eps)[0],
            fy(l1l, l1h, l2l, l2h, cx, cy, u, p, eps=eps)[0],
            m(l1l, l1h, l2l, l2h, cx, cy, u, p, eps=eps)[0]))
...
def calc_mmax_analytic(u, fn, l1, l2):
    # this works for centered object in l1/2 and l2/2 suposing center of
    # rotation is in 0,0
    p = fn / (l1 * l2)
    mmax = u * p * (
        (l2**3) * arcsinh(l1 / abs(l2)) +
        (l1**3) * arcsinh(l2 / l1) + 2 * l1 * l2 * sqrt((l2**2) + (l1**2))
    ) / 12
    return (mmax)
...
#            arguments.append((1,1,cx,cy,1,1))
    ...
    #        for cx in arange(-cxrange,cxrange,step):
    #            list.append([fx(1,1,cx,cy,1,1)[0], fy(1,1,cx,cy,1,1)[0], m(1,1,cx,cy,1,1)[0]]) # noqa
    #        print list
    return (list)
...
def Rz(angle):
    return (
        array(
            [
                [cos(angle), -sin(angle), 0.0], [sin(angle),
                                                 cos(angle), 0], [0, 0, 1]
            ]))
...
def create_data2(angle_steps_dir, angle_steps_rad, l1l, l1h, l2l, l2h, u, den):
    ...
    for a in arange(0.0, 2 * pi, 2. * pi / (angle_steps_dir)):  # cor direction
        for b in arange(pi / 2. / angle_steps_rad, pi / 2.,
                        pi / 2. / angle_steps_rad):  # cor radius
            cx = ((
                (l1h - l1l) / 2.) * cos(a) / tan(b)) + (l1h - l1l) / 2. + l1l
        ...
        temp.append(
            dot(
                ...
                dot(
                    ...
                    dot(
                        Rx(-inc_angle * pi / 180.),
                        dot(Rz(-dir_angle * pi / 180.0), array(i))))))
...
def main2():
    ...
    for num, coord in enumerate(coords):
        ...
        # wframe=ax.plot_wireframe(array([a]),array([b]),array([c]),rstride=1, cstride=1) # noqa
    ...
    os.system(
        "mencoder 'mf://_tmp*.png' -mf type=png:fps=10?"
        " -ovc lavc -lavcopts vcodec=wmv2 -oac copy -o animation%d.mpg"
        % (time.time()))
