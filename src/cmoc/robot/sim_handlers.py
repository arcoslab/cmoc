...
class Joint_sim():
    def __init__(self, portbasename, jointsimportbasename):
        ...
        self.torque_out_port.open(
            portbasename + '/joint_sim_client/torque_out')
        yarp.Network.connect(
            portbasename + '/joint_sim_client/torque_out',
            jointsimportbasename + '/torque_in')
...
class Finger_sim():
    def __init__(self, portbasename, finger, fingersimbaseportname):
        ...
        yarp.Network.connect(
            basename + '/torque_out', fingersimbaseportname + '/torque_in')
...
class Hand_sim():
    def __init__(self, handedness, sahand_port):
        ...
        for i in xrange(4):
            self.fingers_sim_client.append(
                Finger_sim(
                    '/torque_sim_' + handedness, i,
                    '/sahand' + str(sahand_port) + '/finger' + str(i)))
        # self.fingers_sim_client=map(Finger_sim, [['/torque_sim_'+handedness,i,'/sahand'+str(sahand_port)+'/finger'+str(i)] for i in xrange(4)]) # noqa
...
class Force_handle(object):
    def __init__(self, baseportname, remote_name):
        self.port = new_port(
            baseportname + '/force:i', 'in', remote_name, timeout=10.0)
    ...
    def get_pose(
            self, finger, update=True, blocking=True, extra_tool=identity(4)):
        if update:
            self.update_data(blocking)
        finger_data = array(self.data[finger])
        return (
            dot(
                quat.to_matrix_fixed(finger_data[3:7], r=finger_data[:3]),
                extra_tool))
