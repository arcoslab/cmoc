...
import yarp
# from hands_kin import fingers_lim,fingers_coupling, motor_locked_joints,sensor_locked_joints, sensor_finger_coupling # noqa
...
class Finger_controller_cart_pos(object):
    ...
    def __init__(self, parent):
        ...
        # TODO: set nice reference for finger( the best would be current first
        # position
        ...
        self.controller_joint_velocity = Finger_controller_joint_vel(
            self.parent)
        ...
        print numpy.concatenate(
            (self.ref.reshape(16), numpy.array([self.slow_down_distance])))
        self.vector_field.setParams(
            numpy.concatenate(
                (self.ref.reshape(16),
                 numpy.array([self.slow_down_distance]))))
        self.control_signal_jpos = array([0.0] * self.parent.num_joints)
        self.control_signal_jvel = array(
            [0.0] * self.parent.torque_controlled_num_joints)
        self.control_signal_jstiff = array(
            [0.0] * self.parent.torque_controlled_num_joints)
    ...
    def update_control_signals(self):
        """
        Control signal calculation, should calculate position, velocity,
        stiffness (optional)
        """
        # Put your control algorithm here. fill in control_signal_jpos,
        # control_signal_jvel, control_signal_jstiff signals get current
        # cartesian position
        self.parent.kinematics.get_cart_pos()
        self.cur_frame_np = kdl_frame_to_numpy_array(
            self.parent.kinematics.cur_frame_kdl)
        self.cur_frame_np.resize(16)
        pos = self.vector_field.getVector(
            self.cur_frame_np)[:3] * self.vector_field.getScalar(
                self.cur_frame_np)[0]
        #        print pos
        rot = self.vector_field.getVector(self.cur_frame_np)[
            3:] * self.vector_field.getScalar(self.cur_frame_np)[1]
        #        print rot
        ...
        if self.parent.kinematics.calc_vik(self.out_cart_vel) < 0:
            ...
            #            print 'Number of joints', self.parent.kinematics.motor_chain.getNrOfUnlockedJoints() # noqa
            #            print 'Qdot out', kdl_jntarray_to_numpy_array(self.parent.kinematics.qdot_out_kdl) # noqa
            self.controller_joint_velocity.set_refs(
                dot(
                    pinv(self.parent.kinematics.coupling_matrix),
                    kdl_jntarray_to_numpy_array(
                        self.parent.kinematics.qdot_out_kdl)))
            self.controller_joint_velocity.update_control_signals()
            self.control_signal_jpos = (
                self.controller_joint_velocity.control_signal_jpos)
            self.control_signal_jvel = (
                self.controller_joint_velocity.control_signal_jvel)
            self.control_signal_jstiff = (
                self.controller_joint_velocity.control_signal_jstiff)
...
class Finger_controller_joint_vel(object):
    ...
    def set_refs(self, ref):
        """
        Sets the joint velocity controller reference values ref: python list or
        numpy array of joint velocities [(thumb_base(opt)), base, proximal,
        distal] units: radians/s
        """
    ...
    def update_control_signals(self):
        """
        Control signal calculation, should return 6 or 9 parameters:
        position, velocity, stiffness (optional)
        """
        # Put your control algorithm here. fill in control_signal_jpos,
        # control_signal_jvel, control_signal_jstiff signals This controller
        # just sends extreme positions with a variable velocity. For zero
        # velocity it must tell the finger to go where it is right now.
        self.control_signal_jpos = self.control_signal_jpos
        biggers = numpy.greater(
            self.ref, array([0.0] * self.parent.num_joints))
        smallers = numpy.less(self.ref, array([0.0] * self.parent.num_joints))
        for i, (less, greater, ref) in enumerate(zip(smallers, biggers,
                                                     self.ref)):
            if ref != 0:
                if less:
                    self.control_signal_jpos[
                        i] = self.parent.kinematics.limits[i][0]
                if greater:
                    self.control_signal_jpos[
                        i] = self.parent.kinematics.limits[i][1]
            else:
                self.control_signal_jpos[i] = self.parent.cur_joint_pos[
                    i]  # NEW, test
...
class Finger_controller_sahand_joint_pos(object):
    ...
    def __init__(self, parent):
        """
        Parent can be used in the controller for reading current data (with
        sahand controllers this is not necessary)
        """
        ...
        joint_pos_controller_vel = array(
            [50.0 * pi / 180.0] * parent.torque_controlled_num_joints)
        joint_pos_controller_stiffness = array(
            [0.01 * 180.0 / pi] * parent.torque_controlled_num_joints)
        self.params = concatenate(
            (joint_pos_controller_vel, joint_pos_controller_stiffness))
        self.name = 'sahand'
        self.control_stiffness = False

    def set_refs(self, ref, offsets=False):
        """
        Sets the position controller reference values (for the internal hand
        position controller)
        ref: python list or numpy array of joint angles. [base,proximal,distal]
             (for all but thumb finger)
        units: radians
        """
        if len(ref) == self.parent.num_joints:
            if offsets:
                # print 'ref', ref, 'offsets', self.parent.angle_offsets
                self.ref = array(ref) + self.parent.angle_offsets
                # Be careful could be + for some other reasons.
    ...
    def update_control_signals(self):
        """
        Control signal calculation, should return 6 or 9 parameters:
        position, velocity, stiffness (optional) or one more if the finger is
        the thumb
        """
        # Put your control algorithm here. fill in control_signal_jpos,
        # control_signal_jvel, control_signal_jstiff signals
        # 4 values for thumb , 3 for other fingers
        self.control_signal_jpos = self.ref
...
class Kinematics(object):
    """
    This deals with the finger kinematics, forces, speeds, and positions in
    cartesian are here calculated, also the inverses of this
    """
    ...
    def __init__(self, parent, handedness):
        ...
        ch = self.config_hand
        # from hands_kin import hands_kin, fingers_lim, fingers_coupling, motor_locked_joints, sensor_locked_joints, sensor_finger_coupling # noqa
        ...
        self.motor_locked_joints = ch.motor_locked_joints[
            self.parent.num_finger]
        self.sensor_locked_joints = ch.sensor_locked_joints[
            self.parent.num_finger]
        ...
        self.motor_chain.setCoupling(
            self.motor_locked_joints, self.coupling_matrix.tolist())

        # chain sensor
        self.sensor_chain = Chain(self.motor_chain)
        self.sensor_chain.setCoupling(
            self.sensor_locked_joints, self.sensor_finger_coupling.tolist())
        ...
        self.cur_jvel_kdl = JntArrayVel(self.motor_chain.getNrOfJoints())
        # TODO for the thumb we don't have joint velocity yet, it has to be
        # calculated.
        ...
        self.vik_solver = ChainIkSolverVel_wdls(
            self.motor_chain, self.smallest)
        ...
        self.static_weight_right = identity(
            self.sensor_chain.getNrOfIndJoints())
        self.static_weight_right = identity(
            self.sensor_chain.getNrOfUnlockedJoints())

    def get_cart_pos(self, segment_num=-1):
        if self.pfk_solver.JntToCart(self.cur_jpos_kdl, self.cur_frame_kdl,
                                     segment_num) < 0:
            print 'Error in cart pos calculation'

    def get_cart_vel(self):
        # TODO: not tested yet
        #        for i,(angle,speed) in enumerate(zip(self.parent.cur_joint_pos,self.parent.cur_joint_vel)): # noqa
        #            self.cur_jpos_kdl[i]=angle
        #            self.cur_jvel_kdl.q[i]=angle
        #            self.cur_jvel_kdl.qdot[i]=speed
        if self.vfk_solver.JntToCart(self.cur_jvel_kdl,
                                     self.cur_framevel_kdl) < 0:
    ...
    def calc_vik(self, tw_list):
        if len(tw_list) == 6:
            for i in xrange(6):
                self.tw[i] = tw_list[i]
            if self.vik_solver.CartToJnt(self.cur_jpos_kdl, self.tw,
                                         self.qdot_out_kdl) < 0:
    ...
    def update_cur_jvel_kdl(self):
        ...
        temp = concatenate(
            (
                self.parent.cur_joint_vel,
                array([self.parent.cur_joint_vel[-1]])))
    ...
    def update_cur_jpos_kdl(self):
        ...
        temp = concatenate(
            (
                self.parent.cur_joint_pos,
                array([self.parent.cur_joint_pos[-1]])))
        for i, locked in enumerate(self.sensor_locked_joints):
            if locked:
    ...
    def get_forces(self):
        ...
        if self.jac_solver_sensors.JntToJac(self.cur_jpos_kdl,
                                            self.jac_sensors, True) < 0:
        ...
        try:
            self.forces = dot(
                weighted_pseudo_inverse(
                    jac_sensors_np.T, self.static_weight_right,
                    self.static_weight_left), self.parent.cur_torques)
        except Exception, e:
            print(
                'Error during force calculation,'
                ' force value not updated. Error:', e
            )
...
class Finger(object):
    def __init__(self, config_hand, num_finger, handedness):
        ...
        self.torque_calibration_factors = array(
            [1.0] * self.torque_controlled_num_joints)
    ...
    def update_cur_joint_pos(self, pos, offset=True):
        # This function is called automatically by the hand object when new
        # data is retrieved.
    ...
    def update_cur_torques(self, torques, factors=True):
        ...
        #        self.cur_torques=array([torques[0]*self.base_torque_factor,torques[1]*self.proximal_torque_factor*0.72,torques[2]*self.distal_torque_factor])# 0.62 # noqa
    ...
    def get_cart_pos(self, segment_num=-1):
        ...
        for row in xrange(3):
            for col in xrange(4):
                self.cur_cart_pos[row, col] = self.kinematics.cur_frame_kdl[
                    row, col]
    ...
    def get_forces(self):
        ...
        returns: array([forcex,forcey,forcez,torquex,torquey,torquez])
                 (wrench?)
        units: N, Nm
        """
        # TODO
...
def controller_f(
    ...
    hand_port_ctrl.open(
        portprefix + client_port_name_suffix + str(sahand_number) + '/ctrl')
    yarp.Network.connect(
        sahand_port_name_prefix + str(sahand_number) + '/out',
        portprefix + client_port_name_suffix + str(sahand_number))
    yarp.Network.connect(
        portprefix + client_port_name_suffix + str(sahand_number),
        sahand_port_name_prefix + str(sahand_number) + '/in')
    yarp.Network.connect(
        portprefix + client_port_name_suffix + str(sahand_number) + '/ctrl',
        sahand_port_name_prefix + '/cmd')
    ...
    def update_sensor_data(fingers, hand_port):
        ...
        """
        # TODO for left hand we have to change the sign of the base angle for
        # all the fingers to match the kinematic description.
        #        print 'test here'
        # init_time=time.time()
        ...
        if pendingreads > 0:
            ...
            for fingerbottle, finger in zip(map(yarp.Value.asList, map(
                    bottle.get, range(2, bottle.size()))), all_finger_list):
                if finger == THUMB:
                    thumb_angle = bottle.get(
                        THUMB_ANGLE_FIELD).asDouble()  # for thumb position
                    thumb_speed = 0.0  # TODO: Calculate thumb speed
                    fingers[finger].update_cur_joint_pos(
                        [thumb_angle] + map(
                            yarp.Value.asDouble,
                            map(fingerbottle.get, angles_field)))
                    fingers[finger].update_cur_joint_vel(
                        [thumb_speed] + map(
                            yarp.Value.asDouble,
                            map(fingerbottle.get, speeds_field)))
                    # TODO: check if the thumb angle value should go at the
                    # beggining or at the end of the joint array
                else:
                    fingers[finger].update_cur_joint_pos(
                        map(
                            yarp.Value.asDouble,
                ...
                fingers[finger].update_cur_torques(
                    array(
                        map(
                            yarp.Value.asDouble,
    ...
    def send_ctrl_signal_server(
            fingers, hand_port, stiffness=False, list_fingers=[]):
        if len(list_fingers) > 0:
            ...
            for finger in list_fingers:
                ...
                # for transmiting also stiffness TODO: use control signal here
                # now
                # data=fingers[finger].controller.ref[:3].tolist()+fingers[finger].controller.params.tolist()
                if stiffness:
                    data = (
                        fingers[finger].controller.control_signal_jpos.tolist(
                        )[-3:] + fingers[finger].controller.
                        control_signal_jvel.tolist() + fingers[finger]
                        .controller.control_signal_jstiff.tolist())
                else:
                    data = (
                        fingers[finger].controller.control_signal_jpos.tolist(
                        )[-3:] + fingers[finger]
                        .controller.control_signal_jvel.tolist())
                for item in data:
                    cmd_bottle_finger_data.addDouble(item)
                if finger == THUMB:
                    # seting thumb angle
                    cmd_bottle_finger.addDouble(
                        fingers[finger].controller.control_signal_jpos[0])
...
#            print 'Bottle', cmd_bottle.toString()
    ...
    while True:
        new_cmd = False
        if all([fingers[num_finger].controller.name == 'sahand'
                for num_finger in xrange(num_fingers_from_server)]):
            # print 'All fingers are set to sahand controller'
            cmd = cmd_queue.get(True)
            # blocks completely because all controllers are sahand and
            #  sahand control is done inside the hand
        ...
        if new_cmd:
            ...
            if cmd[0] == 'update_sensor_data':
                ...
                #                data=[]
                #                for num_finger in xrange(num_fingers_from_server): # noqa
                ...
                for num_finger in xrange(num_fingers_from_server):
                    if num_finger == 0:
                        out_data[num_finger, 0, :] = fingers[
                            num_finger].get_joint_pos()
                        out_data[num_finger, 1, :] = fingers[
                            num_finger].get_joint_vel()
                    else:
                        out_data[num_finger, 0, :3] = fingers[
                            num_finger].get_joint_pos()
                        out_data[num_finger, 1, :3] = fingers[
                            num_finger].get_joint_vel()
                    out_data[num_finger, 2, :3] = fingers[
                        num_finger].get_torques()

                # ack_queue.put(['update_sensor_data_finished',out_data])
                ack_queue.put(out_data)

            elif cmd[0] == 'set_controller':
                # cmd[1] is finger number, cmd[2] is an initialize object
        ...
        for num_finger in xrange(num_fingers_from_server):
            if fingers[num_finger].controller.name != 'sahand':
                fingers[num_finger].controller.update_control_signals(
                )  # control processing is done here
                send_ctrl_signal_server(
                    fingers,
                    hand_port,
                    fingers[num_finger].controller.control_stiffness,
                    list_fingers=[num_finger])  # send signals to server
            else:
                # don't do anything since control signals are sent when the
                # references or parameters are updated
                pass
        ...
        #        print 'All controllers sahand?', all([fingers[num_finger].controller.name=='sahand' for num_finger in xrange(num_fingers_from_server)]) # noqa
        if (wait_time < 0.0) or (all(
            [fingers[num_finger].controller.name == 'sahand'
             for num_finger in xrange(num_fingers_from_server)])):
            # print 'Desired control cycle not attained, use a slower control
            # cycle or get a faster computer' # this is wrong FIX TODO
            wait_time = 0.0
        last_time = new_time
...
class Hand(object):
    """
    Lets controls the kimp robot hands it starts another process which is
    responsible for the control and comunication of the robot when we select to
    use the sahand internal controller, the other process only relays the
    information from here. This class creates an object for each finger, each
    finger have an associated controller, each finger can have a different
    ...
    def __init__(
        ...
        self.controller_p = Process(
            target=controller_f,
            args=(
                self.cmd_queue, self.ack_queue, handedness, self.sahand_number,
                portprefix, sahand_port_name_prefix, self.config_hand))
        self.handedness = handedness
        self.fingers = []
        ...
        for num_finger in xrange(self.num_fingers_from_server):
            self.fingers.append(
                Finger(self.config_hand, num_finger, handedness))
...
#        self.fingers_state.append([])
    ...
    def pause_controller(self, finger_list=all_finger_list):
        ...
        for finger in finger_list:
            if self.fingers[finger].controller.get_name() == 'sahand':
                print(
                    'This controller is always running just call control'
                    ' when you want to set a new reference')
            else:
                print(
                    'For other types of controllers we have to'
                    'start a processes that calls control continuously')
                # TODO
    ...
    def update_sensor_data(self):
        # updates the joint values on all the fingers from the controller
        # process
        init_time = time.time()
        ...
        for num_finger in xrange(self.num_fingers_from_server):
            finger_data = result
            if num_finger == 0:
                self.fingers[num_finger].update_cur_joint_pos(
                    finger_data[num_finger, 0], offset=False)
                self.fingers[num_finger].update_cur_joint_vel(
                    finger_data[num_finger, 1])
            else:
                self.fingers[num_finger].update_cur_joint_pos(
                    finger_data[num_finger, 0, :3], offset=False)
                self.fingers[num_finger].update_cur_joint_vel(
                    finger_data[num_finger, 1, :3])
            self.fingers[num_finger].update_cur_torques(
                finger_data[num_finger, 2, :3], factors=True)
        # else:
        #    print 'Error' # TODO
    ...
    def update_controller_refs(self, list_fingers=all_finger_list):
        """
        Sends the current local reference values to the controller process
        for the selected fingers
        """

        for num_finger in list_fingers:
            self.cmd_queue.put(
                [
                    'update_ref', num_finger,
                    self.fingers[num_finger].controller.ref
                ])

    def update_angle_offsets(self, list_fingers=all_finger_list):
        """
        Sends the current local reference values to the controller process for
        the selected fingers
        """
        for num_finger in list_fingers:
            self.cmd_queue.put(
                [
                    'update_angle_offsets', num_finger,
                    self.fingers[num_finger].angle_offsets
                ])

    def update_controller_params(self, list_fingers=all_finger_list):
        """
        Sends the current local parameter values to the controller process
        for the selected fingers
        """
        for num_finger in list_fingers:
            self.cmd_queue.put(
                [
                    'update_params', num_finger,
                    self.fingers[num_finger].controller.params
                ])
...
# We make the hand object with the finger inside. We can enable or disable
# individual fingers during the constructor, disabled fingers means that
# Hand_client will not transmit data to control this fingers, receiving still
# happens. We get data from the fingers by reading from the hand the data:
# hand.update_joints() or hand.update_cart() (update cart does an update_joints
# internally) and then accessing the finger data with:
# hand.fingers[thumb].get_joint_pos() hand.fingers[thumb].get_joint_torque()
# hand.fingers[thumb].get_cart_pos() hand.fingers[thumb].get_force() (can also
# add for cartesian and joint velocity) There is a question open: When we
# update, should we update the hand server data only or some of the calculated
# data also (depending on a given list)? To move the fingers we first set the
# desired position/velocity/torque/force with:
# hand.fingers[thumb].set_joint_pos() .set_cart_pos() .set_joint_vel()
# .set_cart_vel() .set_torque() .set_force() and then we call hand.move().
# There will be a different controller for each finger.
# The selected controller is determined by the last sent command.
