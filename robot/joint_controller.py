...
from numpy import array, amin, amax, pi, finfo
import numpy 
...
class Controller_mass_sim(object):
    ...
    def update(self,time):
        ...
        #if self.ks1 element is smaller than 1. then multiply by torque_int, otherwise multiply self.ks2 element with ext_torque
        for i,num in enumerate(self.ks1.tolist()):
            if num<1.0:
                ...
                ks1[i]=1.0
                ks2[i]=self.ks2[i]
        #print "Torque int", self.torque_int, "Torque ext", self.ext_torque, "ks", ks1, ks2
        self.acc=(ks1*self.torque_int+self.ext_torque*ks2)/self.inertia
        #print "Acc", self.acc
        #motion equations
        self.q=self.q+self.qv*self.delta_time+self.acc*(self.delta_time**2)/2.0
        self.qv=self.qv+self.acc*self.delta_time
        #Check for joint limits
        q_unlimited=self.q
        self.q=amin(array([amax(array([self.low_limits,self.q]),0),self.high_limits]),0)
        #speed must be zero for limited joints
        changed=q_unlimited-self.q
        for i in xrange(self.joint_num):
            if changed[i]!=0.:
                self.qv[i]=0.
        #print "qv", self.qv
